...
int main (int argc, char *argv[])
{
   ...
   std::cout <<  finder1.GetElem() << std::endl;
   auto obj = finder1.GetGSLIBElem();

   // for (int p = 0; p < obj.Size(); p++)
  // std::cout << obj[p] << std::endl;

  
   //finder1.FindPoints(vxyz);
   auto obj_ref = finder1.GetReferencePosition();
  // auto obj_ref = finder1.GetGSLIBReferencePosition();

  // for (int p = 0; p < obj_ref.Size(); p++)
    //   std::cout << obj_ref[p] << std::endl;

;
   for (int i = 0; i < mesh_1.GetNE(); i++)
       std::cout <<  (*mesh_1.GetElement(i)).GetNEdges() << std::endl;

   FiniteElementSpace * fespace  = mesh_1.GetNodes()->FESpace();
   ...
   for (int i = 0; i < mesh_1.GetNBE(); i++)
   {
      ...
      const FiniteElement * elem = fespace->GetBE(i);
      const IntegrationRule & ir = elem->GetNodes();

      int count = ir.Size();
      std::cout << i << " count = " << count << std::endl;

      FaceElementTransformations * T = mesh_1.GetBdrFaceTransformations(i);
      DenseMatrix P;
      T->Transform(ir, P);
      ...
      for (int j = 0; j < ir.Size(); j++){
         T->SetIntPoint(&ir[j]);
         IntegrationPoint p = T->GetElement1IntPoint();
   ...
   for (int i = 0; i < pts_cnt; i++)
      std::cout << obj[i] << " " << obj_ref(i*dimension + 0) << "  " << obj_ref(i*dimension + 1) << std::endl;
